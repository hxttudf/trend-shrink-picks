...
def main():
    ...
    # 窗口内买点
    rows = picks.execute(
        "SELECT symbol, name, signal_type, signal_date FROM chanlun_signals "
        "WHERE status='ok' AND signal_type LIKE '%买%' AND signal_date >= date('now', ?)",
        (f'-{WIN} day',)).fetchall()
    # 先清窗口内标记
    ...
    cache = {}
    n_d3 = n_w30 = 0
    for sym, name, typ, sdate in rows:
        if sym not in cache:
            k = seq.execute(
    ...
    picks.close()
    seq.close()
    print(f"✅ D3/W30标记: 窗口{WIN}天 | 买点{len(rows)}条 | D3标记{n_d3}条 / W30标记{n_w30}条 | {time.time()-t0:.0f}s")
