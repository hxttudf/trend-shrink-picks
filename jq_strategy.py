...
def screen_stocks(context):
    ...
    for stock in all_stocks:
        ...
        df = attribute_history(stock, 60, '1d',
                               ['close', 'volume'], df=True, skip_paused=True)
        ...
        if cond_b or cond_ultra:
            ...
            candidates.append({'stock':stock,'name':name,'strategy':strategy,
...
def trade(context):
    ...
    for stock in list(context.portfolio.positions.keys()):
        ...
        if info['days'] >= g.hold_days:
            ...
            log.info(f'[{today}] 卖出 {stock} 持有{info["days"]}d')
